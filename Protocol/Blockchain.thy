...
next
  ...
  qed
qed
lemma valid_t_blocks : assumes "valid_t (Node m l r)" shows ""
...
proof(cases "l")
  ...
  next
    case (Node x21 x22 x23)
    then show ?thesis using assms lleaf apply(simp add: GenBlock_def tree0_def set_def) apply(auto) sorry
next
  case (Node x21 x22 x23)
