...
-- maybe try import Test.QuickCheck.Instances.Natural
...
less_int :: Integer -> Integer -> Bool;
less_int k l = if (k < l) then True else False;
...
equal_int :: Integer -> Integer -> Bool;
equal_int k l = if (k == l) then True else False;

less_eq_int :: Integer -> Integer -> Bool;
less_eq_int k l = if (k <= l) then True else False;
}
